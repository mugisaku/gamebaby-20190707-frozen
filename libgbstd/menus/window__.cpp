...
get_width() const noexcept
{
  return m_style_pointer->m_left_side_width
        +(m_content_pointer? m_content_pointer->get_width():0)
...
get_height() const noexcept
{
  return m_style_pointer->m_top_width
        +(m_content_pointer? m_content_pointer->get_height():0)
...
void
frame::
draw(const gbstd::canvas&  cv) const noexcept
{
  ...
  int  w = get_width();
    ...
    if(m_assembler_pointer)
    {
      auto&  as = *m_assembler_pointer;
        ...
        {
          gbstd::canvas  tmp_cv(cv,m_position.x,
                                   m_position.y+m_style_pointer->m_top_width+cont_ptr->get_height(),
                                   w,
                                   m_style_pointer->m_bottom_width);
        ...
        if(as.m_right_side)
        {
          gbstd::canvas  tmp_cv(cv,m_position.x+m_style_pointer->m_left_side_width+cont_ptr->get_width(),
                                   m_position.y+m_style_pointer->m_top_width,
                                   m_style_pointer->m_right_side_width,
                                   cont_ptr->get_height());
