...
void
draw_sv_entry(const gbstd::menus::entry&  ent, pair&  p, const gbstd::canvas&  cv) noexcept
{
  ...
  auto&  cur = view.get_first_cursor();
    ...
    if(!gbstd::test_input_barrier())
    {
        ...
        if(gbstd::g_input.test_up())
        {
          ...
          man.clear_and_push_text(cur.get_data<pair>().description);
        }
...
initialize_menu() noexcept
{
  m_first_menu_table.resize(1,(sizeof(g_table)/sizeof(*g_table)))
              .set_entry_width(gbstd::g_font_width*10)
