...
main_loop() noexcept
{
  sdl::update_control();
    ...
    {
      g_screen_canvas.copy_canvas({g_root.get_image()},0,0);
...
main(int  argc, char**  argv)
{
  ...
  auto&  chrsel_frame = g_root.create_frame().set_content(chrsel_table).set_string(u"character table");
